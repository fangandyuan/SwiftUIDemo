...
extension PopoverAnimator: UIViewControllerTransitioningDelegate, UIViewControllerAnimatedTransitioning {
    ...
    // 实现代理方法, 告诉系统谁来负责转场动画,可以控制弹出的页面的大小以及位置还有添加背景色
    ...
    func animationController(forDismissed dismissed: UIViewController) -> UIViewControllerAnimatedTransitioning?
    {
        isPresent = false
        // 发送通知, 通知控制器即将消失
        NotificationCenter.default.post(name: NSNotification.Name(rawValue: PopoverAnimatorWilldismiss), object: self)
        return self
    }
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        if isPresent
        {
            ...
            //将要展示的view添加到transitionContext容器上
            // 注意: 一定要将视图添加到容器上
